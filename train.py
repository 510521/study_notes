...
class LLM(PreTrainedModel):
    ...
    def get_input_embeddings(self):
        return self.tokon_embeddings

    def set_input_embeddings(self, new_embeddings):
        self.tokon_embeddings = new_embeddings
...
if __name__ == '__main__':   
    ...
    # 2. 数据集加载与切分 (关键修改点)
    full_dataset = LLMDataset('pretrain.jsonl', tokenizer=tokenizer, max_seq_len=512)
     # 1. 计算长度
    dataset_len = len(full_dataset)
    # 2. 计算切分数量 (90% 训练, 10% 验证)
    # 注意：int() 是向下取整，保证两个数加起来不超过总长度
    train_size = int(0.9 * dataset_len)
    eval_size = dataset_len - train_size 
    
    # 3. 执行切分
    # random_split 会返回一个列表，里面是两个新的 Subset 数据集

    train_dataset, eval_dataset = random_split(full_dataset, [train_size, eval_size], generator=torch.Generator().manual_seed(42))
    
    print(f"数据集切分完成：训练集 {len(train_dataset)} 条, 验证集 {len(eval_dataset)} 条")

     # 3. 训练参数配置 (关键修改点：加入验证和动态调整)
    args = TrainingArguments(output_dir='./kimi', 
                            ...
                            logging_steps=1,
                            # --- 验证集相关配置 ---
                            eval_strategy="epoch",      # 每个 epoch 结束后评估验证集
                            per_device_eval_batch_size=16,    # 验证时的 batch size
                            # --- 模型保存与早停配置 ---
                            save_strategy="epoch",            # 每个 epoch 保存一次
                            load_best_model_at_end=True,      # 训练结束时加载验证集 Loss 最低的模型
                            metric_for_best_model="eval_loss",# 监控验证集 Loss
                            greater_is_better=False,          # Loss 越小越好
                            save_total_limit=2,   # 只保留最近的 2 个检查点，省空间
    ...
    trainer = Trainer(
        ...
        eval_dataset=eval_dataset,     # 验证集 (关键：传入验证集)
        processing_class=tokenizer,
        data_collator=data_collator
    )
    # 如果是初次训练resume_from_checkpoint为false，接着checkpoint继续训练，为True
    trainer.train(resume_from_checkpoint=False)
    trainer.save_model('./saves/kimi_best')
    trainer.save_state()
    ...
    if len(train_losses) > 0:
        ...
        if eval_losses:
            # 计算 eval 的 step 位置（通常在 epoch 结尾）
            eval_steps = [i for i, log in enumerate(logs) if 'eval_loss' in log]
